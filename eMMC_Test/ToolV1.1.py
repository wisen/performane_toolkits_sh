...
class FragmentUI:
	...
	def stop_wr_threads(self, dev_sn):
		return
		
	#devices{"sn1":{"s":"online","fg":0.6,"frag":0.63,"stime":xxxxx,"func":"off"}}
	def wr_func(self,dev_sn,path):
		cmd_str="while [ 1 ]; do if [ ! -f "+path+" ];then mkdir -p "+path+";fi;"+"_tmp_size=$(($RANDOM%99+1));dd if=/dev/urandom of="+path+"/$(($RANDOM*1000+$RANDOM))_${_tmp_size}k.dat bs=${_tmp_size}k count=1 status=none;sync;done &"
		...
		while self.devices[dev_sn]["s"] == "online" and self.devices[dev_sn]["wrfn"] == "on":
			...
			try:
				proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
				#proc = subprocess.check_call(cmd, shell=False)
			except OSError as e:
				print(e)
	...
	def del_func(self,dev_sn,path):
		...
		cmd_str="while [ 1 ];do rm -rf "+self.path+self.dir+"$(($RANDOM%99+1))/*_$(($RANDOM%99+1))k.dat;sync;done"
		cmd = ["adb", "-s", dev_sn, "shell", cmd_str]
	...
	def create_wr_thread(self,dev):
		...
		for i in range(self.wrthreads):
			...
			#t.start()
			self.wr_func(dev, self.path+self.dir+str(i))
		return
